"""
Intent Tensor Theory - Self-Resolving Tensor Field V9
Fixed pattern priority and stricter detection
...
class IntentTensorFieldV9:
    ...
    def _detect_fill(self):
        ...
        for seed in self.d0:
            ...
            # 1. ENCLOSED (binary_fill_holes) - check FIRST
            if np.any(inp != 0):
        ...
        # Priority order: enclosed > crossfill > connect_row > connect_col > connect_row_or_col > bbox
    ...
    def _get_connect_row_positions(self, inp):
        """Get expected positions for connect_row pattern."""
        nz_pos = np.argwhere(inp != 0)
        expected = set()
    ...
    def _get_connect_col_positions(self, inp):
        """Get expected positions for connect_col pattern."""
        nz_pos = np.argwhere(inp != 0)
        expected = set()
    ...
    def _detect_color(self):
        for seed in self.d0:
            inp, out = seed['inp'], seed['out']
            nz = inp[inp != 0]
            if len(nz) > 0 and len(set(nz)) == 1:
                c = nz[0]
                if np.array_equal(np.where(inp == 0, c, 0), out):
                    return {'type': 'invert', 'color': int(c)}
        return None
    ...
    def _detect_expansion(self):
        ...
        if s['h_ratio'] == 2 and s['w_ratio'] == 2:
            ...
            rot_tile = np.vstack([
                np.hstack([inp, np.fliplr(inp)]),
                np.hstack([np.fliplr(inp), inp])
            ])
            if np.array_equal(rot_tile, out):
                patterns['rot_tile_2x2'] = True
        ...
        for key in list(patterns.keys()):
            for seed in self.d0:
                ...
                elif key == 'rot_tile_2x2':
                    exp = np.vstack([np.hstack([i, np.fliplr(i)]), np.hstack([np.fliplr(i), i])])
                else:
                    continue
    ...
    def collapse(self):
        ...
        if self.d2.get('color'): return self._collapse_color()
    ...
    def _collapse_color(self):
        c = self.d2['color']
        if c['type'] == 'invert':
            col = c['color']
            self.transform = lambda x, c=col: np.where(np.array(x) == 0, c, 0)
            self.solution_type = 'invert'
        self.d3 = {'collapsed': True}
        return self
    ...
    def _collapse_expand(self):
        ...
        elif 'rot_tile_2x2' in e:
            def fn(x):
                i = np.array(x)
                return np.vstack([np.hstack([i, np.fliplr(i)]), np.hstack([np.fliplr(i), i])])
            self.transform = fn
            self.solution_type = 'rot_tile_2x2'
