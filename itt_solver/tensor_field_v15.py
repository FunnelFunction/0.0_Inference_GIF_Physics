...
class IntentTensorFieldV15:
    ...
    def compute_d2(self):
        ...
        if self.d1['same_shape']:
            ...
            # Check gravity
            gravity = self._detect_gravity()
            if gravity:
                self.d2['gravity'] = gravity
                return self
            
            # Check checkerboard (2-row input -> checkerboard)
            checker = self._detect_checkerboard()
            if checker:
                self.d2['checkerboard'] = checker
                return self
            ...
            # Check crop_bbox (output is cropped bounding box of input)
            crop = self._detect_crop_bbox()
            if crop:
                self.d2['crop_bbox'] = crop
                return self
    ...
    def _check_self_tile(self):
        """Check if output is input tiled at non-zero positions."""
        for seed in self.d0:
            inp, out = seed['inp'], seed['out']
            h, w = inp.shape
            
            if out.shape != (h * h, w * w):
                return False
    ...
    def _check_mode_downscale(self, scale):
        for seed in self.d0:
            inp, out = seed['inp'], seed['out']
            if out.shape[0] * scale != inp.shape[0] or out.shape[1] * scale != inp.shape[1]:
                return False
            
            result = np.zeros_like(out)
            for r in range(out.shape[0]):
                for c in range(out.shape[1]):
                    block = inp[r*scale:(r+1)*scale, c*scale:(c+1)*scale]
    ...
    def _detect_crop_bbox(self):
        """Detect if output is input cropped to bounding box of non-zero pixels."""
        for seed in self.d0:
            inp, out = seed['inp'], seed['out']
    ...
    def _detect_checkerboard(self):
        """Detect 2-row uniform -> checkerboard pattern."""
        for seed in self.d0:
            inp, out = seed['inp'], seed['out']
            
            # Input should be 2 rows of uniform colors
            if inp.shape[0] != 2:
                return None
            ...
            if len(c1_vals) != 1 or len(c2_vals) != 1:
                return None
            
            c1, c2 = list(c1_vals)[0], list(c2_vals)[0]
            
            # Output should be checkerboard
            expected = np.zeros_like(out)
            for r in range(out.shape[0]):
                for c in range(out.shape[1]):
                    expected[r, c] = c1 if (r + c) % 2 == 0 else c2
            
            if not np.array_equal(expected, out):
                return None
        
        return {'type': 'checkerboard_2row'}
    
    def _detect_gravity(self):
        """Detect gravity pattern (pixels fall in a direction)."""
        for direction in ['down', 'up', 'left', 'right']:
            valid = True
            for seed in self.d0:
                inp, out = seed['inp'], seed['out']
                expected = self._apply_gravity(inp, direction)
                if not np.array_equal(expected, out):
                    valid = False
                    break
            if valid:
    ...
    def _apply_gravity(self, arr, direction):
    ...
    def _detect_fill(self):
        ...
        for seed in self.d0:
            ...
            # 3b. CONNECT_SAME_COLOR_ROW (only connect if row endpoints have same color)
            same_row_expected = self._get_connect_same_color_row_positions(inp)
            # For connect_same_row, verify fill color matches row endpoint
            same_row_match = same_row_expected == new_set and len(new_set) > 0
            if same_row_match:
                # Verify each fill uses its row's endpoint color
                for r in range(inp.shape[0]):
                    row_inp = inp[r, :]
                    row_out = out[r, :]
                    nz_cols = np.where(row_inp != 0)[0]
                    if len(nz_cols) >= 2:
                        c_min, c_max = nz_cols.min(), nz_cols.max()
                        if row_inp[c_min] == row_inp[c_max]:
                            expected_color = row_inp[c_min]
                            for c in range(c_min + 1, c_max):
                                if row_inp[c] == 0 and row_out[c] != expected_color:
                                    same_row_match = False
                                    break
                    if not same_row_match:
                        break
            results['connect_same_row'] = same_row_match
        ...
        # connect_same_row doesn't use global offset - check it first
        if all(r.get('empty') or r.get('connect_same_row') for r in pattern_results):
            return {'type': 'connect_same_row', 'color_offset': 0}  # Uses per-row color
    ...
    def _get_connect_same_color_row_positions(self, inp):
        """Get positions to fill for rows where both endpoints have the same color."""
        expected = set()
        for r in range(inp.shape[0]):
            row = inp[r, :]
            ...
            if len(nz_cols) >= 2:
                c_min, c_max = nz_cols.min(), nz_cols.max()
                # Check if endpoints have same color
    ...
    def _detect_expansion(self):
        ...
        # Check downscale (output smaller than input)
        if s['h_ratio'] < 1 and s['w_ratio'] < 1:
            inv_h = 1 / s['h_ratio']
            inv_w = 1 / s['w_ratio']
            if inv_h == inv_w == int(inv_h):
                scale = int(inv_h)
                # Try mode-based downscale
                if self._check_mode_downscale(scale):
                    patterns['downscale'] = scale
        ...
        # Check self_tile (n x n -> n*n x n*n where tiled at non-zero positions)
        if s['h_ratio'] == s['w_ratio'] == s['inp'].shape[0] and s['h_ratio'] == int(s['h_ratio']):
            if self._check_self_tile():
                patterns['self_tile'] = True
    ...
    def collapse(self):
        ...
        if self.d2.get('crop_bbox'): return self._collapse_crop_bbox()
        if self.d2.get('checkerboard'): return self._collapse_checkerboard()
        if self.d2.get('gravity'): return self._collapse_gravity()
    ...
    def _collapse_crop_bbox(self):
        ...
        self.transform = fn
        self.solution_type = 'crop_bbox'
        self.d3 = {'collapsed': True}
        return self
    
    def _collapse_checkerboard(self):
        def fn(x):
            arr = np.array(x)
            c1, c2 = arr[0, 0], arr[1, 0]
            result = np.zeros_like(arr)
            for r in range(arr.shape[0]):
                for c in range(arr.shape[1]):
                    result[r, c] = c1 if (r + c) % 2 == 0 else c2
            return result
        self.transform = fn
        self.solution_type = 'checkerboard_2row'
        self.d3 = {'collapsed': True}
        return self
    
    def _collapse_gravity(self):
        direction = self.d2['gravity']['direction']
        def fn(x, d=direction):
            return self._apply_gravity(np.array(x), d)
        self.transform = fn
        self.solution_type = f'gravity_{direction}'
        self.d3 = {'collapsed': True}
        return self
            ...
            def fn(x):
                ...
                for r in range(arr.shape[0]):
                    ...
                    if len(nz_cols) >= 2:
                        c_min, c_max = nz_cols.min(), nz_cols.max()
                        if row[c_min] == row[c_max]:
                            fc = row[c_min]  # Use row's own color
                            for c in range(c_min + 1, c_max):
                                if result[r, c] == 0:
                                    result[r, c] = fc
                return result
    ...
    def _collapse_expand(self):
        ...
        if 'upscale' in e:
            ...
            def fn(x):
                ...
                return result
            self.transform = fn
            self.solution_type = 'self_tile'
        elif 'downscale' in e:
            scale = e['downscale']
            def fn(x, s=scale):
                arr = np.array(x)
                out_h, out_w = arr.shape[0] // s, arr.shape[1] // s
                result = np.zeros((out_h, out_w), dtype=arr.dtype)
                for r in range(out_h):
                    for c in range(out_w):
                        block = arr[r*s:(r+1)*s, c*s:(c+1)*s]
                        vals, counts = np.unique(block, return_counts=True)
                        result[r, c] = vals[np.argmax(counts)]
                return result
            self.transform = fn
            self.solution_type = f'downscale_{scale}x'
        elif 'tile' in e:
            f = e['tile']
